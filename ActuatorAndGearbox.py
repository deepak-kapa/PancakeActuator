...
# base directory for results (absolute path)
...
class compoundPlanetaryGearbox:
    ...
    def meshingConstraint(self):
        ...
        Ns2 = self.Nr - self.NpSmall * 2
        return ((self.Ns % self.numPlanet == 0) and (self.Nr % (self.numPlanet) == 0)) and ((self.Nr + Ns2) % (self.numPlanet * 2) == 0)
    ...
    def noPlanetInterferenceConstraint(self):
        ...
        return (2 * (Rs + Rp1) * np.sin(np.pi/(2 * numPlanet)) - Rp1 - sCarrierExtrusionRadiusMM >= self.sCarrierExtrusionClearanceMM) and (2 * (Rr - Rp2) * np.sin(np.pi/(2 * numPlanet * 2)) - Rp2 >= 0.5)
...
#=========================================================================
#-------------------------------------------------------------------------
# Pancake Compound Actuator class
#-------------------------------------------------------------------------
class pancakeCompoundActuator:
    def __init__(self, 
                 design_parameters,
                 motor_driver_params,
                 motor                    = motor, 
    ...
    def getToothForces(self, constraintCheck=True):
        ...
        Ft_rp = ((self.serviceFactor*self.motor.getMaxMotorTorque()) * RpBig_Mt) / (numPlanet * 2 * Rs_Mt * RpSmall_Mt)
    ...
    def getMassKG_3DP(self):
        ...
        # Density
        #-----------------------------------------
        density_gear = self.compoundPlanetaryGearbox.densityGears
        density_structure = self.compoundPlanetaryGearbox.densityStructure
        ...
        sunFwM     = sunFwMM     / 1000.0 # TODO: check the order of the index order should be always sun, planet1, planet2, ring
        planet1FwM = planet1FwMM / 1000.0
        planet2FwM = planet2FwMM / 1000.0
        ringFwM    = ringFwMM    / 1000.0
        ...
        Motor_case_mass = Motor_case_volume * density_structure
        ...
        ring_mass = ring_volume * density_gear

        if (density_gear != density_structure):
            gearbox_casing_mass = ring_volume * (density_structure + density_gear) + (bearing_holding_structure_volume + case_mounting_structure_volume + large_fillet_volume) * density_structure
        else: gearbox_casing_mass = ring_volume * density_gear + (bearing_holding_structure_volume + case_mounting_structure_volume + large_fillet_volume) * density_structure
        ...
        carrier_mass = carrier_volume * density_structure
        ...
        sun_mass         = sun_volume * density_gear
        ...
        sun_dummy_dia_mm = (Nr - 2*Np2) * module2
        sun_dummy_volume = (np.pi * ((sun_dummy_dia_mm*0.5) ** 2) * (planet2FwMM * 0.45) * 1e-9)
        planet_mass   = (planet1_volume + planet2_volume * 2 + sun_dummy_volume) * density_gear ## for pancake
        ...
        sec_carrier_mass   = sec_carrier_volume * density_structure
        ...
        bearing_retainer_mass   = bearing_retainer_volume * density_structure
...
#========================================================================
#------------------------------------------------------------
# Class: Compound Planetary Actuator
#------------------------------------------------------------
class compoundPlanetaryActuator:
    ...
    def getMassKG_3DP(self):
        ...
        planet1_volume = (np.pi * ((DiaPlanet1MM*0.5)**2 - (planet_bore*0.5)**2) * planet1FwMM) * 1e-9
        planet2_volume = (np.pi * ((DiaPlanet2MM*0.5)**2 - (planet_bore*0.5)**2) * planet2FwMM) * 1e-9
        planet_mass   = (planet1_volume + planet2_volume) * density_gear
...
class optimizationCompoundPlanetaryActuator:
    ...
    def printOptimizationResults(self, Actuator=compoundPlanetaryActuator, log=1, csv=0):
        ...
        if log:
            ...
            Sun_mass = Actuator.sun_mass
            Planet_mass_total = Actuator.planet_mass * numPlanet
            Ring_mass = Actuator.ring_mass
            Gearbox_casing_mass = Actuator.gearbox_casing_mass
            Motor_case_mass = Actuator.Motor_case_mass
            print(iter, ",", gearRatio, ",", moduleBig, ",", moduleSmall, ",", Ns, ",", NpBig, ",", NpSmall, ",", Nr, ",", numPlanet, ",", fwSunMM, ",", fwPlanetBigMM, ",", fwPlanetSmallMM, ",", fwRingMM, ",", mass, ",", eff, ",", peakTorque, ",", Cost, ",", tooth_forces[0], ",", tooth_forces[1], ",", Torque_Density, ",", Outer_Bearing_mass, ",", Actuator_width, ",", Sun_mass, ",", Planet_mass_total, ",", Ring_mass, ",", Gearbox_casing_mass, ",", Motor_case_mass)
    ...
    def getResultFilePrefix(self, Actuator):
        material_mode = getattr(Actuator, "materialMode", "3dp")
        if type(Actuator) is pancakeCompoundActuator:
            if material_mode == "metal":
                return "Pancake_Metal_CPG"
            return "Pancake_CPG"
        if material_mode == "metal":
            return "Metal_CPG"
        return "CPG"
    ...
    def optimizeActuatorWithoutPSC(self, Actuator=compoundPlanetaryActuator, log=1, csv=0, printOptParams=1):
        ...
        prefix = self.getResultFilePrefix(Actuator)
        if csv:
            fileName = os.path.join(RESULTS_DIR, f"results_BruteForce_{Actuator.motor.motorName}",
                                    f"{prefix}_BRUTEFORCE_{Actuator.stressAnalysisMethodName}_{Actuator.motor.motorName}.csv")
        elif log:
            fileName = os.path.join(RESULTS_DIR, f"results_BruteForce_{Actuator.motor.motorName}",
                                    f"{prefix}_BRUTEFORCE_{Actuator.stressAnalysisMethodName}_{Actuator.motor.motorName}_LOG.txt")
        ...
        with open(fileName, "w") as file1:
            ...
            if log:
                ...
                print("iter, gearRatio, moduleBig, moduleSmall, Ns, NpBig, NpSmall, Nr, numPlanet, fwSunMM, fwPlanetBigMM, fwPlanetSmallMM, fwRingMM, mass, eff, peakTorque, Cost, tooth_forces_sp, tooth_forces_rp, Torque_Density, Outer_Bearing_mass, Actuator_width, Sun_mass, Planet_mass_total, Ring_mass, Gearbox_casing_mass, Motor_case_mass")
                                            ...
                                            if (Actuator.compoundPlanetaryGearbox.geometricConstraint() and 
                                                ...
                                                if (Actuator.compoundPlanetaryGearbox.gearRatio() >= self.gearRatioIter and 
                                                    ...
                                                    if self.Cost < MinCost:
                                                        ...
                                                        opt_actuator = type(Actuator)(design_parameters        = self.design_parameters,
    ...
    def optimizeActuatorWithPSC(self, Actuator=compoundPlanetaryActuator, log=1, csv=0):
        ...
        prefix = self.getResultFilePrefix(Actuator)
        if csv:
            fileName = os.path.join(RESULTS_DIR, f"results_bilevel_{Actuator.motor.motorName}",
                                    f"{prefix}_BILEVEL_{Actuator.stressAnalysisMethodName}_{Actuator.motor.motorName}.csv")
        elif log:
            fileName = os.path.join(RESULTS_DIR, f"results_bilevel_{Actuator.motor.motorName}",
                                    f"{prefix}_BILEVEL_{Actuator.stressAnalysisMethodName}_{Actuator.motor.motorName}_LOG.txt")
        # ensure output directory exists
        os.makedirs(os.path.dirname(fileName), exist_ok=True)
        with open(fileName, "w") as file1:
            sys.stdout = file1
            ...
            if log:
                ...
                # Printing the optimization iterations below
                print(" ")
                print("iter, gearRatio, moduleBig, moduleSmall, Ns, NpBig, NpSmall, Nr, numPlanet, fwSunMM, fwPlanetBigMM, fwPanetSmallMM, fwRingMM, PSCs, PSCp1, PSCp2, PSCr, CD_SP, CD_PR, mass, eff, peakTorque, Cost, tooth_forces_sp, tooth_forces_rp, Torque_Density, Outer_Bearing_mass, Actuator_width, Sun_mass, Planet_mass_total, Ring_mass, Gearbox_casing_mass, Motor_case_mass")
                                                ...
                                                if (Actuator.compoundPlanetaryGearbox.gearRatio() >= self.gearRatioIter and 
                                                    ...
                                                    if self.Cost < MinCost:
                                                        ...
                                                        opt_planetaryGearbox = compoundPlanetaryGearbox(design_parameters         = self.design_parameters,
                                                                                                        ...
                                                                                                        fwRingMM                  = Actuator.compoundPlanetaryGearbox.fwRingMM, # mm
                                                                                                        maxGearAllowableStressMPa = Actuator.compoundPlanetaryGearbox.maxGearAllowableStressMPa) # MPa) # kg/m^3
                                                        opt_actuator = pancakeCompoundActuator(design_parameters        = self.design_parameters,
                                                                                                 motor                    = Actuator.motor,
                                                                                                 compoundPlanetaryGearbox = opt_planetaryGearbox,
                                                                                                 FOS                      = Actuator.FOS,
                                                                                                 serviceFactor            = Actuator.serviceFactor,
